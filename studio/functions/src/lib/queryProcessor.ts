...
export class QueryProcessor {
  ...
  generateAlternativeQueries(query: string): string[] {
    ...
    if (lowerQuery.includes('qr') || lowerQuery.includes('merchant')) {
      alternatives.push(
        `IQR interoperable QR codes ${query}`,
        `merchant payment acceptance QR codes ${query}`,
        `digital payments infrastructure ${query}`
      );
    }
    
    if (lowerQuery.includes('payment') && !lowerQuery.includes('g2p')) {
      alternatives.push(
        `payment infrastructure interoperability ${query}`,
        `UPI digital payments ${query}`
      );
    }
    ...
    // Add DPI-specific expansions for all queries
    if (!lowerQuery.includes('dpi') && !lowerQuery.includes('digital public infrastructure')) {
      alternatives.push(`DPI ${query}`);
    }
